...
/**
 * Isla premium pintada (WebP) con fallback SVG procedural si falla la carga.
 */
export function AdventureMapPaintedIsland({ worldId, className }: Props) {
  const { reduceMotion } = useMotionSafe();
  const [webpFailed, setWebpFailed] = useState(false);
  const config = getPaintedIslandConfig(worldId);
  const webpSrc = ADVENTURE_MAP_ASSETS.islandWebp(worldId);
  const fullBleed = config?.paintedArt ?? false;
