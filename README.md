...
Para producción, genera el build con `npm run build` y sirve la carpeta **`dist/`** (no el código fuente). Las variables `VITE_SUPABASE_URL` y `VITE_SUPABASE_ANON_KEY` deben existir en el entorno de build. El backend sigue en Supabase.
...
**Manual (GitHub Pages sin Actions):** genera el build con `VITE_BASE_PATH=/<nombre-repo>/ npm run build` para que salgan rutas tipo **`/<repo>/assets/...`**.

### Vercel (u otro hosting en la raíz)

El sitio vive en **`https://tu-proyecto.vercel.app/`** (sin `/atenas/` en la URL). El **`.env.production` del repo ya no fija** `VITE_BASE_PATH=/atenas/`: si el build se hace con esa base, el HTML pedirá **`/atenas/assets/...`** y en Vercel esos archivos **no existen** (están en **`/assets/...`**) → errores MIME.

- En el panel de **Vercel → Project → Settings → Environment Variables**, no pongas `VITE_BASE_PATH` o déjala en **`/`**.
- Haz un **nuevo deploy** (o “Redeploy”) tras el cambio. El `build-info.json` en la raíz debería mostrar **`"viteBase":"/"`** si lo añades en CI similar a GitHub.

### Errores MIME en producción

1. **GitHub Pages:** URL **`https://<usuario>.github.io/<repo>/`**. Si el build usó base **`/`** pero entras con `/repo/`, o al revés, falla.
2. **Vercel en raíz:** el build debe usar base **`/`** (no `/atenas/`).
3. **Ctrl+F5** o incógnito (caché).
4. En **GitHub Actions**, el paso **“Comprobar rutas en index.html”** valida Pages; en Vercel revisa `dist/index.html` tras build local con la misma config que el hosting.
5. **`build-info.json`** (solo si lo genera tu CI): en GitHub Pages debería verse `"viteBase":"/<repo>/"`; en Vercel en la raíz, **`"viteBase":"/"`**.
